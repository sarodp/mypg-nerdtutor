# http://www.nerdparadise.com/programming/pythontetris
...
def mn(e,s,a,o,ox,oy,fc,ck,cn,l,dr,j,t,f):
	while not('12-Quit'in e or"y': 27,"in e):
		fc,d,y,e=fc+1+5*key.get_pressed()[274]+l/15,
		((l/10)%7),
		'0201120 0020110 2001022 0222101 1120011 0000202'.split(),
		str(v(lambda x:x.type!=3,event.get()))
  
		c=[[0]*3,[z]*3]
		+[m(lambda x:127*int(y[:3][x][d]),b(3)),
		m(lambda x:127*int(y[3:][x][d]),b(3))]
		...
		for y in b(200):
			...
			dr(s,c[1],Rect(j,t,151,301),1)
			s.blit(f.render("Lines: "+str(l),0,c[1]),(j,t/2))
			display.flip()
			ck.tick(60)
