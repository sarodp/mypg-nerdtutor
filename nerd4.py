...
def create_background(width, height):
	...
	while y < height:
		x = 0
		while x < width:
			row,col = y// tile_width, x // tile_width
			xcolor =colors[ (row + col) % 2]
			xrect = pygame.Rect(x, y, tile_width, tile_width)
			pygame.draw.rect(background, xcolor, xrect)
			x += tile_width
		y += tile_width
	return background
...
def do_rectangle_demo(surface, counter):
	width,height = 60,60
	left = (counter*5 // 4) % (surface.get_width()-width*.9)
	top = (counter*5 // 3) % (surface.get_height()-height*.9)
...
def do_line_demo(surface, counter):
	...
	for con in connections:
		...
		lwidth = 5
		pygame.draw.line(surface, color, (x1, y1), (x2, y2), lwidth)
